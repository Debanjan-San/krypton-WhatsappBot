...
module.exports = MessageHandler = async (messages, client) => {
    try {
        ...
        if (!isGroup && !isCmd) console.log(color('~', 'yellow'), color('RECV', 'green'), color('Message', 'yellow'), 'from', color(sender.split('@')[0], 'yellow'), 'args :', color(args.length, 'blue'))
        if (isCmd && isGroup) console.log(color('~', 'yellow'), color('EXEC', 'red'), color(cmdName, 'yellow'), 'from', color(sender.split('@')[0], 'yellow'), 'in', color(gcName, 'yellow'), 'args :', color(args.length, 'blue'))
        if (!isCmd && isGroup) console.log(color('~', 'yellow'), color('RECV', 'green'), color('Message', 'yellow'), 'from', color(sender.split('@')[0], 'yellow'), 'in', color(gcName, 'yellow'), 'args :', color(args.length, 'blue'))
        ...
        const command =
            ...
            client.cmd.find((cmd) => cmd.aliases && cmd.aliases.includes(cmdName))
